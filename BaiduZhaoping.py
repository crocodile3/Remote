...
start_url = "http://zhaopin.baidu.com/api/qzasync?query={}&city={}&pn={}"
headers = {
            # "Referer": "",
...
def get_total(city,keyword):
    ...
    :return:
    """
    url = start_url.format(keyword,city,str(0))
...
def get_detail_url(city,keyword,pages):
    url = start_url.format(keyword,city,str(pages*10))
    # 更新请求头
    ua = UserAgent()
...
def get_content_list(html_str):
    ...
    job_name = html.xpath("//h4[@class='job-name']/text()")[0] if html.xpath("//h4[@class='job-name']/text()") else None
    salary = html.xpath("//span[@class='salary']/text()")[0] if html.xpath("//span[@class='salary']/text()") else None
    ...
    job_place = html.xpath("//p[contains(text(),'工作地点')]/text()")[0] if html.xpath("//p[contains(text(),'工作地点')]/text()") else None
    job_describe = html.xpath("//div[@class='job-detail']/p/text()")[0] if html.xpath("//div[@class='job-detail']/p/text()") else None
    # job_resp = re.search()
    job_addr = html.xpath("//div[@class='job-addr']/p[2]/text()")[0] if html.xpath("//div[@class='job-addr']/p[2]/text()") else None
    gs_name = html.xpath("//div[@class='item-bd']/h4/text()")[0] if html.xpath("//div[@class='item-bd']/h4/text()") else None
    origin = html.xpath("//div[@class='item-bd']/h4/text()")[0] if html.xpath("//div[@class='item-bd']/h4/text()") else None
    job = dict(
        job_name = job_name,
        salary =salary,
        education = education,
...
def run(pages):
    # 1 爬取列表页
    urls = get_detail_url(city,keyword,pages)
    # 2 爬取详情页
    if urls:
        for url in urls:
            parse_url(url)
...
if __name__ == '__main__':
    ...
    jobs_num, pages = get_total(city, keyword)
    print("{}地区共搜索到{}个{}相关职位".format(city, jobs_num, keyword))
    pool = Pool()
    pool.map(run,[i * 10 for i in range(pages)])
